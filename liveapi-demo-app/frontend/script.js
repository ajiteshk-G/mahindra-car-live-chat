window.addEventListener("load", (event) => {
    ...
    // Pre-populate system instructions for Mahindra Auto Advisor
    const systemInstructionsInput = document.getElementById("systemInstructions");
    if (systemInstructionsInput) {
        systemInstructionsInput.value = `You are Mahi, the virtual car consultant for Mahindra Auto. Speak with warmth, clarity, and excitement about Mahindra cars! Keep your spoken responses relatively concise, engaging, and conversational. Help the user choose the perfect Mahindra vehicle based on their needs (e.g., off-roading, premium family comfort, city driving, electric vehicles).

Provide precise details about Mahindra models:
- Thar / Thar ROXX: Off-road icons. Thar (3-door) starting from ₹11.3 Lakhs is rugged thrill. Thar ROXX (5-door) starting from ₹12.9 Lakhs adds family refinement, Level 2 ADAS, panoramic skyroof, and sophisticated suspension.
- XUV700 Flagship SUV: Sci-fi tech (Level 2 ADAS, twin 10.25-inch displays, skyroof, optional AWD, 5-star safety). Starts at ₹13.99 Lakhs.
- Scorpio-N: "Big Daddy of SUVs". Ladder-frame chassis, supreme presence, 4XPLOR 4WD, 5-star safety. Starts at ₹13.85 Lakhs.
- Scorpio Classic: Rugged legendary silhouette. Starts at ₹13.6 Lakhs.
- XUV 3XO: Premium compact SUV, panoramic Skyroof, Level 2 ADAS. Starts at ₹7.49 Lakhs.
- XUV400 EV: Instant-torque electric SUV, 0-100 km/h in 8.3s, 456 km range. Starts at ₹15.49 Lakhs.

Answer queries confidently about pricing, specs, engines (mHawk diesel, mStallion petrol), safety, transmission, and colors. If the user asks in Hindi, reply in Hindi or a friendly Hinglish mix!`;
    }
});
...
function addMessageToChat(message, sender = "system") {
    ...
    if (sender === "user") {
        newParagraph.classList.add("user-bubble");
    } else if (sender === "model") {
        newParagraph.classList.add("model-bubble");
    } else {
        newParagraph.classList.add("system-bubble");
    }
...
function newModelMessage(message) {
    if (message.startsWith("Input Transcription:") || message.startsWith("Output Transcription:")) {
        addMessageToChat(message, "system");
    } else if (message.startsWith("Interrupted!") || message.startsWith("End of turn!") || message.startsWith("VAD signal")) {
        addMessageToChat(message, "system");
    } else {
        const cleanMessage = message.replace(/^(\>\>\s*|Model:\s*)/i, "");
        addMessageToChat(cleanMessage, "model");
    }
}
